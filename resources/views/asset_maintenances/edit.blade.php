      ...
      <div class="box-body">
        ...
        <div class="form-group {{ $errors->has('user_id') ? ' has-error' : '' }}">
          <label for="user_id" class="col-md-3 control-label">
            {{ trans('admin/asset_maintenances/table.admin_name') }}
          </label>
          <div class="col-md-7{{  (\App\Helpers\Helper::checkIfRequired($item, 'user_id')) ? ' required' : '' }}">
            @if ($selectedAsset == null)
            {{ Form::select('user_id', $admin_list , Input::old('user_id', $item->user_id), ['class'=>'select2', 'style'=>'min-width:350px']) }}
            @else
            {{ Form::select('user_id', $admin_list , Input::old('admin_id', $selectedAsset), ['class'=>'select2', 'style'=>'min-width:350px', 'enabled' => 'false']) }}
            @endif
